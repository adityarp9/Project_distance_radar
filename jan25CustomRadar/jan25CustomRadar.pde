...
void setup() {
 ...
 smooth();
 myPort = new Serial(this,"/dev/cu.usbmodem1421", 9600); // starts the serial communication
 myPort.bufferUntil('\n'); // reads the data from the serial port up to the character '.'. So actually it reads this: angle,distance.
 orcFont = loadFont("Monospaced-48.vlw");
}
void draw() {
...
void drawText() { // draws the texts on the screen
  ...
  textSize(25);
  fill(98,245,60);
  translate((width-width*0.5104)+width/2*cos(radians(0)),(height-height*0.0574)-width/2*sin(radians(0)));
  rotate(radians(-90));
  text("0°",0,0);
  resetMatrix();
  ...
  text("150°",0,0);
  resetMatrix();
  translate((width-width*0.5104)+width/2*cos(radians(180)),(height-height*0.0574)-width/2*sin(radians(180)));
  rotate(radians(-90));
  text("180°",0,0);
  popMatrix(); 
}
